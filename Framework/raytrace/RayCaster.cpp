...
float3 RayCaster::compute_pixel(unsigned int x, unsigned int y) const
{
  ...
  Ray& ray = camera->get_ray(make_float2((x+0.5f)*win_to_ip.x-0.5f, (y+0.5f)*win_to_ip.y-0.5f));
  HitInfo& hit = HitInfo();
  ray.tmax = RT_DEFAULT_MAX;
  // In the closest hit function of Accelerator
  if (scene->closest_hit(ray, hit)) {
	  return get_shader(hit)->shade(ray, hit, true);
  }
  else {
	  return get_background(ray.direction);
  }
  return (ray.direction + 1.0) / 2.0;
}
