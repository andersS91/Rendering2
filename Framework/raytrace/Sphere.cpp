...
bool Sphere::intersect(const Ray& r, HitInfo& hit, unsigned int prim_idx) const
{
	...
	float tmark = -bhalf - sqrt(d);

	if (tmark > r.tmax || tmark < r.tmin) {		
			tmark = -bhalf + sqrt(d);
			if (tmark > r.tmax || tmark < r.tmin) {
				return false;
			}
	...
	////SUCCESS
	hit.has_hit = true;
	hit.geometric_normal = normalize(r.origin + r.direction*tmark - center);
	hit.shading_normal = normalize(r.origin + r.direction*tmark - center);
